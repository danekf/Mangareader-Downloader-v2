...
#from my experience, manga reader uses the same name and just changes the last digits to choose the chapter. This implementation allows us to choose input the url WITHOUT the final integers, and then specify the starting chapter and end chapter we want.
inputURL = sys.argv[1]
startVolStr = sys.argv[2]
endVolStr = sys.argv[3]

currentInt = int(startVolStr)
endInt = int(endVolStr)


while currentInt <= endInt:
  url = inputURL + str(currentInt)
  main()
  currentInt += 1
